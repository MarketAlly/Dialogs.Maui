...
{
    /// <summary>
    /// Displays a time picker dialog for selecting times
    /// </summary>
    public class TimePickerDialog : BaseDialog
    {
        private readonly TaskCompletionSource<TimeSpan?> _taskCompletionSource = new();
        ...
        /// <param name="initialTime">Initial time to display</param>
            ...
            DialogType dialogType = DialogType.None)
        {
            ...
            _selectedTime = initialTime ?? DateTime.Now.TimeOfDay;
            ...
            var timePickerBorder = new Border
            {
            ...
            pickerRow.Add(timePickerBorder, 0, 0);

            if (_nowButton != null)
            {
                pickerRow.Add(_nowButton, 1, 0);
            }
        ...
        private async void OnOkClicked(object? sender, EventArgs e)
        {
            ...
            _taskCompletionSource.TrySetResult(_selectedTime);
        }
        ...
        /// <summary>
        /// Shows a time picker dialog with default settings
        /// </summary>
        public static async Task<TimeSpan?> ShowAsync(
            string title,
            string? description = null,
            TimeSpan? initialTime = null)
        {
            return await ShowAsync(title, description, initialTime, null, null, true, false, DialogType.None);
        }

        /// <summary>
        /// Shows a time picker dialog with full customization
        /// </summary>
        public static async Task<TimeSpan?> ShowAsync(
            string title,
            string? description,
            ...
            DialogType dialogType = DialogType.None)
        {
            // Check if a time picker dialog is already showing
            if (MopupService.Instance.PopupStack.Any(p => p is TimePickerDialog))
                return null;

            var dialog = new TimePickerDialog(
                title, description, initialTime,
        ...
        /// </summary>
        public async Task<TimeSpan?> ShowAsync()
        {
        ...
        /// <summary>
        /// Hides the current time picker dialog
        /// </summary>
        public static async Task HideAsync()
        {
            var dialog = MopupService.Instance.PopupStack.FirstOrDefault(p => p is TimePickerDialog);
            if (dialog != null)
            {
